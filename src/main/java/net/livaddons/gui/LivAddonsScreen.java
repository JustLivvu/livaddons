...
public class LivAddonsScreen extends Screen {
    ...
    private void saveAndSync() {
        ...
        ApiClient.syncProfile(newData).thenAccept(result -> {
            client.execute(() -> {
                this.saveButton.active = true;
                if (result.success()) {
                    PlayerDataManager.getInstance().updateCache(newData);
                    this.statusMessage = Component.literal("§a" + result.message());
                } else {
                    this.statusMessage = Component.literal("§c" + result.message());
                }
            });
        });
    }
