...
<<<<<<< HEAD
    ...
    private void rebuildCommandFields() {
        for (EditBox field : commandFields) removeWidget(field);
        commandFields.clear();
        for (int i = 0; i < FeatureSettings.commandKeybinds().size(); i++) {
            final int index = i;
            FeatureSettings.CommandKeybind binding = FeatureSettings.commandKeybinds().get(i);
            EditBox field = new EditBox(font, popupX() + 88, popupY() + 64 + i * 28,
                    158, 18, Component.literal("Command"));
            field.setMaxLength(256);
            field.setHint(Component.literal("/command"));
            field.setValue(binding.command());
            field.setResponder(value -> FeatureSettings.setCommandKeybind(index,
                    FeatureSettings.commandKeybinds().get(index).keyCode(), value));
...
<<<<<<< HEAD
    public boolean keyPressed(KeyEvent event) {
        if (capturingCommandKey >= 0 && capturingCommandKey < FeatureSettings.commandKeybinds().size()) {
            int key = event.key();
            FeatureSettings.CommandKeybind binding = FeatureSettings.commandKeybinds().get(capturingCommandKey);
            FeatureSettings.setCommandKeybind(capturingCommandKey,
                    key == GLFW.GLFW_KEY_ESCAPE ? -1 : key, binding.command());
            capturingCommandKey = -1;
            return true;
        }
...
<<<<<<< HEAD
            case "Command Keybinds" -> {
                ...
                for (int i = 0; i < FeatureSettings.commandKeybinds().size(); i++) {
                    FeatureSettings.CommandKeybind binding = FeatureSettings.commandKeybinds().get(i);
                    int rowY = cy + 26 + i * 28;
                    renderButton(graphics, cx, rowY, 72,
                            capturingCommandKey == i ? "Press key..." : keyName(binding.keyCode()));
...
<<<<<<< HEAD
            case "Command Keybinds" -> {
                if (inside(mx, my, cx, cy, cw, 18)) {
                    ...
                    for (int i = 0; i < FeatureSettings.commandKeybinds().size(); i++) {
                        int rowY = cy + 26 + i * 28;
                        if (inside(mx, my, cx, rowY, 72, 18)) {
                            capturingCommandKey = i;
                            setFocused(null);
                            break;
                        }
                        if (inside(mx, my, cx + cw - 18, rowY, 18, 18)) {
                            FeatureSettings.removeCommandKeybind(i);
                            capturingCommandKey = -1;
                            rebuildCommandFields();
                            break;
                        }
...
<<<<<<< HEAD
        ...
        graphics.fill(x, y, x + width, y + 18, edge);
        graphics.fill(x + 1, y + 1, x + width - 1, y + 17, body);
...
<<<<<<< HEAD
        for (int i = 0; i < commandFields.size(); i++) {
            ...
            field.setX(x + 76);
            field.setY(y + 26 + i * 28);
            field.setWidth(158);
        }
    }

    private int popupHeight() {
        if ("Party Commands".equals(settingsModule)) return 286;
        if ("Cosmetics".equals(settingsModule)) return 276;
        if ("Command Keybinds".equals(settingsModule))
            return Math.max(86, 70 + FeatureSettings.commandKeybinds().size() * 28);
        return 150;
    }
